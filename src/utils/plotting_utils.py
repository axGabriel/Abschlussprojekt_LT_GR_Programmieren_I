...
def plot_speed_and_distance_profile(speed_profile: list[float], distance_profile: list[float], duration_profile: list[float]):
    assert len(speed_profile) - 1 == len(duration_profile), "Speed profile must be longer by 1"
    assert len(distance_profile) - 1 == len(duration_profile), "Distance profile must be longer by 1"

    t_plot, v_plot, s_plot = [], [], []

    t_plot.append(0.0)
    v_plot.append(speed_profile[0])
    s_plot.append(distance_profile[0])

    t_total = 0.0
    for v, s, d in zip(speed_profile[1:], distance_profile[1:], duration_profile):
        t_plot += [t_total, t_total + d]
        v_plot += [v, v]
        s_plot += [s, s]
    ...
    axV.plot(t_plot[0:], v_plot, "b-", label="Speed v / m/s")
    axS.plot(t_plot[0:], s_plot, "r--", label="Distance s / m")
    axV.set_xlabel("Time $t$ / s")
    axV.set_ylabel("Speed $v$ / m/s", color="b")
    axS.set_ylabel("Distance $s$ / m", color="r")
    axV.grid(True)
