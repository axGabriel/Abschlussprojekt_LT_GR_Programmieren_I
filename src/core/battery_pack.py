...
class BatteryPack:
    ...
    def __init__(
        ...
        if v_min >= v_max:
            logger.error(f"v_min ({v_min}) must be less than v_max ({v_max}).")
    ...
    def _get_ocv(self) -> float:
        """Returns the open circuit voltage (OCV). Overridden by subclasses."""
        return self.v_min + (self.v_max - self.v_min) * self.soc

    def voltage(self, current: float = 0.0, temperature_celsius: float = 20.0) -> float:
        """Return the current voltage of the battery considering temperature and internal resistance."""
        ocv = self._get_ocv()
...
class LiPoBatteryPack(BatteryPack):
    ...
    def _get_ocv(self) -> float:
        """Overridden OCV method using LiPo table lookup."""
        return float(np.interp(self.soc, self._soc_table, self._ocv_table))


class NmcBatteryPack(BatteryPack):
    """Nickel mangan cobalt battery pack, inherits from BatteryPack class"""
    _soc_table = [0.00, 0.04, 0.09, 0.13, 0.17, 0.21, 0.26, 0.30, 0.40, 0.52, 0.64, 0.76, 0.88, 1.00]
    _ocv_table = [32.00, 32.61, 33.17, 33.85, 34.24, 34.66, 35.39, 35.65, 36.65, 37.64, 38.91, 40.14, 41.08, 42.00]
    
    def _get_ocv(self) -> float:
        """Overridden OCV method using NMC table lookup."""
        return float(np.interp(self.soc, self._soc_table, self._ocv_table))